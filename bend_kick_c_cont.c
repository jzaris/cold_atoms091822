...
int main(int argc, char *argv[]){
	...
	if(argc == 3){
		...
		std::ofstream dst(new_name_str, std::ios::binary); //output stream for new output file
		...
		//char *line = NULL;
	        //size_t len = 0;
		f = fopen(new_name, "r");  /*Open output file to set positions and velocities*/
		if (f == NULL)
                	exit(EXIT_FAILURE);
		...
		outfile.open(new_name, ios::app | ios::binary);
...
{
	static double fmm_err = 1.0*pow(10,-4);
...
void damping_force()
{
	...
	for (int i =0; i< particles_global; i++)
	{
		...
		//}
		damping_factor = arr(7,i)*1.0*pow(10,8);
